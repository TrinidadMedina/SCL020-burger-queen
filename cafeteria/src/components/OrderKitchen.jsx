...
export function OrderKitchen({ order, handleReady }) {
  ...
  return (
    <div className="flex flex-col m-4 shadow-lg bg-orange-60 rounded m-2 text-dark-green">
      ...
      <div className=" w-60 flex flex-col h-40 pt-2 justify-between rounded bg-white overflow-auto">
        ...
        <div className="justify-self-end pb-2 content-end self-center"> {order.observaciones && "*" + order.observaciones}</div>
      </div>

      <button onClick={toggleModal} className=' bg-orange-1 hover:bg-dark-green   place-content-center text-white font-bold py-2 px-4 m-4 mt-3 rounded justify-self-end'>Listo</button>
    </div >
  )
}
